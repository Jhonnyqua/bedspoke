...
# ----------------------------------------------------------
# LLAMAR GPT-4o: UNA PÁGINA A LA VEZ
# ----------------------------------------------------------
PROMPT = """Esta es una página de un reporte "Housekeeping Daily Summary".

Extraé CADA propiedad de la página con:
1. La dirección completa (columna IZQUIERDA)
2. El nombre del cleaner asignado (columna DERECHA, bajo "Assigned To")

REGLAS:
- Si el cleaner dice "Unassigned", usá "Unassigned".
- Ignorá encabezados, pies de página, fechas, datos de huéspedes/reservas.
- Si una dirección está cortada (sin suburb o postcode), incluila igual.
- Respondé SOLO con JSON válido, sin texto extra, sin markdown:
[{"address": "dirección completa", "cleaner": "Nombre Cleaner"}]
- Si no hay propiedades en la página, respondé: []"""
...
st.caption("GPT-4o lee cada página visualmente — sin errores de parseo.")
