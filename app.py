...
def create_grouped_excel(igms_csv):
    ...
    df_keys["Simplified"] = df_keys["Property Address"].apply(
        simplify_address_15chars
    )

    merged = pd.merge(df_igms, df_keys,
                      on="Simplified", how="left",
                      suffixes=("_IGMS", "_Key"))

    # Aquí corregimos el KeyError usando row.get()
    ...
    grouped = (df.groupby(["Encargado", "Dirección"], as_index=False)
